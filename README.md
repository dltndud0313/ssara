...
> AIoT 기반 시각장애인 안내 로봇 서비스

시각장애인은 이동 중 장애물, 신호등, 주변 환경 정보를 실시간으로 파악하는 데 어려움이 있습니다. SSARA는 이러한 문제를 해결하기 위해 개발한 AIoT 기반 4족보행 안내 로봇 서비스로, 로봇이 주변 환경을 인식하고 안전한 이동을 지원하는 것을 목표로 합니다.

Vue 프론트엔드, Spring Boot 백엔드, MQTT/WebSocket 통신, ROS2 로봇 환경을 연결하여 실시간 관제와 원격 모니터링 기능을 제공하며, 저는 `gae_ws/src/gae_perception` 영역에서 로봇 인식(Perception) 파이프라인 개발을 담당했습니다.
