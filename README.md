...
반려동물 보호자는 외출 중 반려견의 상태를 실시간으로 확인하기 어렵고, 이상 상황 발생 시 즉각적인 대응에도 한계가 있습니다. GAE는 보호자가 언제 어디서나 반려견의 상태를 확인하고 로봇을 원격 제어할 수 있도록 개발한 AIoT 기반 반려견 돌봄 서비스입니다.

Vue 프론트엔드, Spring Boot 백엔드, MQTT/WebSocket 통신, ROS2 로봇 환경을 연결하여 실시간 관제와 원격 제어 기능을 제공하며, 저는 `gae_ws/src/gae_perception` 영역에서 로봇 인식(Perception) 파이프라인 개발을 담당했습니다.
